...
NSString * InterActiveMap_Wroclaw$ModuleRequireFormat = @"(function(exports){"
...
@implementation InterActiveMap_WroclawObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_interactivemap_wroclaw);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create InterActiveMap_Wroclaw global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "InterActiveMap_Wroclaw" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_interactivemap_wroclaw = [[InterActiveMap_WroclawObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_interactivemap_wroclaw];
	
	NSString *_interactivemap_wroclawNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _interactivemap_wroclawNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_interactivemap_wroclaw valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:InterActiveMap_Wroclaw$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.interactivemap_wroclaw.kroll" 
