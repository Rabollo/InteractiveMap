...
//Rare exceptions to indicate a bug in the _interactivemap_wroclaw code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_interactivemap_wroclaw = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_interactivemap_wroclaw]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.interactivemap_wroclaw.module"
