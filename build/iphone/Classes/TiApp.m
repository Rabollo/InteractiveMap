...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kInterActiveMap_WroclawUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kInterActiveMap_WroclawUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a InterActiveMap_Wroclaw app using the node.js CLI
